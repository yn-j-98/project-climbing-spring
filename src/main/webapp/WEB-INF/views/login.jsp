	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-primary" id="sendPasswordBtn" disabled>임시 비밀번호 전송</button>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	function sendToController(userInfo) {
		...
		$.ajax({
			...
			success : function(response) {
				...
				if (response == "true") {
					sweetAlert_success("로그인 성공!","메인페이지로 이동합니다.");
					// 로그인 후 이동하게 될 URL
					window.location.href = 'main.do';
				} else {
					// DB에 해당 이메일이 존재하지 않는다면 회원가입 페이지로 이동
					sweetAlert_info("회원가입을 해주세요.","회원가입 페이지로 이동합니다.");
					window.location.href = 'join.do?member_id=' + response;
				}
			...
			error : function(error) {
				...
				// 로그인 실패 알랏창 띄움
				sweetAlert_error("로그인에 실패했습니다. 다시 시도해 주세요.","");
			}
		});
	}
	...
	document.getElementById('joinBtn').onclick = function() {
		...
		sweetAlert_info("회원가입 페이지로 이동합니다.","");
