...
@Controller
public class RankingAsyncController {
    ...
    @PostMapping("/crewRankAsync.do")
    public  ResponseEntity<List<MemberDTO>>  crewRank(@RequestBody MemberDTO memberDTO, GradeDTO gradeDTO, Model model) {
       ...
       // model.addAttribute("member_datas",member_datas);
        ...
        return ResponseEntity.ok(member_datas);
    }

    @PostMapping("/personalRankAsync.do")
    public ResponseEntity<List<MemberDTO>> personalRankAync(@RequestBody MemberDTO memberDTO, GradeDTO gradeDTO, Model model) {
        ...
        //model에 요청해서 등급 정보를 전부 불러오고 현재 등급 9개
        List<GradeDTO> grade_datas;
        try {
            grade_datas = this.gradeService.selectAll(gradeDTO);
            System.out.println("Grade data size: " + grade_datas.size());
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("Fetching grade data failed", e);
        }
        System.out.println("너는 실행되 ? " + grade_datas.size());
        System.out.println("너는 왜 0이니?" + grade_datas);
        //불러온 랭킹에 등급 사진을 추가하기 위해 for문을 사용
        for (MemberDTO data : member_datas) {
            int member_total_point = data.getMember_total_point();
            System.out.println("member_total_point" + member_total_point);
