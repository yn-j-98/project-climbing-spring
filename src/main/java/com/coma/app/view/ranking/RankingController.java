...
public class RankingController {
	...
	public String ranking( Model model,MemberDAO memberDAO, MemberDTO memberDTO, GradeDTO gradeDTO, GradeDAO gradeDAO) {
		...
		List<GradeDTO> model_grade_datas = gradeService.selectAll(gradeDTO);
		...
		for (MemberDTO data : member_datas) {
			...
			if(0 <= member_total_point) {
				...
				for (int i = 0; i < model_grade_datas.size(); i++) {
					int grade_min = model_grade_datas.get(i).getGrade_min_point();
					int grade_max = model_grade_datas.get(i).getGrade_max_point();
					...
					if(grade_min <= member_total_point && member_total_point <= grade_max) {
						data.setMember_grade_profile(servletContext.getContextPath()+"/grade_folder/" + model_grade_datas.get(i).getGrade_profile());
						data.setMember_grade_name(model_grade_datas.get(i).getGrade_name());
					}
